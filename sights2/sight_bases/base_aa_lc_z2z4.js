...
let init = ({
	...
	sight.add(new Line({from: [0, 8.25], to: [0, 16.5]}).withMirrored("y"));
	sight.add(new Line({from: [8.25, 0], to: [16.5, 0]}).withMirrored("x"));

	if (showCenteralCircle) {
		sight.circles.addComment("Centeral circle (air 1600m)");
		sight.add(new Circle({
			segment: [25, 65],
			diameter: Toolbox.calcDistanceMil(11, 1600),
			size: 4
		}).withMirroredSeg("xy"));
	}
	...
	let drawCross = (fromVal, toVal) => sight.add([new Line({ from: [0, fromVal], to: [0, toVal] }).withMirrored("y"), new Line({ from: [fromVal, 0], to: [toVal, 0] }).withMirrored("x")]);
	let drawX = (fromRadius, toRadius) => sight.add(new Line({ from: [fromRadius / sqrt2, fromRadius / sqrt2], to: [toRadius / sqrt2, toRadius / sqrt2] }).withMirrored("xy"));
	...
	sight.add(new TextSnippet({
		...
		align: "right",
		pos: [ldnInfo.main.rangeMil.to + 30, 4.5],
		size: 1
	}));

	sight.circles.addComment(`Leading value circles - ${assumeTgtSpeedMain}km/h`);
	sight.add(new Circle({ diameter: ldnInfo.main.mil * 2, size: 2.2 }));
	sight.add(new Circle({ diameter: ldnInfo.sub.mil * 2, size: 1.6 }));
	...
	for (let ldnRange of [ldnInfo.main.rangeMil, ldnInfo.sub.rangeMil]) {
		drawCross(ldnRange.from, ldnRange.to);
		drawX(ldnRange.from, ldnRange.to)
	}

	sight.lines.addComment("Outer cross from screen borders");
	drawCross(ldnInfo.main.rangeMil.to + 20, 450);
